<script>
		...
		if (type === 'calendar:alert' && data) {
			...
			if ($isLastActiveTab) {
				if ($settings?.notificationEnabled ?? false) {
					new Notification(`${data.title} • SocialForge`, {
						body: timeStr,
		...
		if ((event.chat_id !== $chatId && !$temporaryChatEnabled) || isInBackground) {
			if (type === 'chat:completion') {
				...
				if (done) {
					...
					if ($isLastActiveTab) {
						if ($settings?.notificationEnabled ?? false) {
							new Notification(`${displayTitle} • SocialForge`, {
								body: content,
								icon: `${WEBUI_BASE_URL}/static/favicon.png`
							});
						}
	...
	const channelEventHandler = async (event) => {
		...
		if ((!channel || isInBackground) && event?.user?.id !== $user?.id) {
			...
			if (type === 'message') {
				...
				if ($isLastActiveTab) {
					if ($settings?.notificationEnabled ?? false) {
						new Notification(`${title} • SocialForge`, {
							body: data?.content,
	...
	const windowMessageEventHandler = async (event) => {
		if (
			!['https://socialforge.pro', 'https://www.socialforge.pro', 'http://localhost:9999'].includes(
				event.origin
			)
		) {
			return;
